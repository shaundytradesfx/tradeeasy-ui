...
const AlertForm = ({ onCreateAlert }: { 
  ...
  return (
    <div className="mb-8 bg-gray-800/50 backdrop-blur-lg border border-gray-700 p-6 rounded-xl shadow-lg">
      <h3 className="text-xl font-medium mb-4">Create Alert</h3>
...
export default function Alerts() {
  ...
  const handleCreateAlert = (asset: string, threshold: number, direction: 'above' | 'below') => {
    ...
    if (result.success && result.alert) {
      ...
      // For demonstration purposes, simulate a triggered alert after a delay
      setTimeout(() => {
        const triggeredAlert = generateSampleAlert(asset, threshold, direction);
        setAlerts(prev => [triggeredAlert, ...prev]);
        
        // Show notification for triggered alert
        toast.success(`New alert triggered for ${asset}!`, {
          icon: '🔔',
          duration: 5000
        });
      }, 5000);
